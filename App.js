...
function QuickPhotoMeasure({job,onBack,onUpdate,onSave,onFinish}){
  ...
  const addPhoto=async(source='camera')=>{if(job?.photos?.length)await onSave?.(job,true);let r;if(source==='camera'){const perm=await ImagePicker.requestCameraPermissionsAsync();if(!perm.granted){Alert.alert('Câmera','Autorize o acesso à câmera para fazer a medição rápida.');return;}r=await ImagePicker.launchCameraAsync({mediaTypes:['images'],quality:.58,base64:true});}else r=await ImagePicker.launchImageLibraryAsync({mediaTypes:['images'],quality:.58,base64:true});if(r?.canceled)return;const a=r.assets[0],photo={id:uid(),uri:a.uri,dataUri:a.base64?`data:${a.mimeType||'image/jpeg'};base64,${a.base64}`:null,marks:[],createdAt:Date.now()};const next={...job,photos:[...(job.photos||[]),photo],updatedAt:Date.now()};onUpdate(next);setActivePhoto(photo);resetView()};

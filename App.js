...
const GROUPS = [
  ...
  { key: 'Estruturas', icon: '▦', items: ['Pilar', 'Coluna', 'Shaft', 'Viga'] },
  { key: 'Equipamentos', icon: '▣', items: ['Pia', 'Cuba', 'Geladeira', 'Frigobar', 'Freezer', 'Adega', 'Fogão', 'Cooktop', 'Forno', 'Micro-ondas', 'Lava-louças', 'Máquina de lavar', 'Secadora', 'Tanque', 'Coifa', 'Depurador', 'Ar-condicionado', 'Televisão', 'Mesa', 'Sofá', 'Cama solteiro', 'Cama casal', 'Beliche', 'Outro', ...RESERVED_SPACE_TYPES] },
...
const FREE_PLAN_TYPES = ['Mesa','Sofá','Cama solteiro','Cama casal','Beliche'];
...
function Home({ projects, onNew, onOpen, onDelete, onClients, onHelp, onMore }) {
  const [query,setQuery]=useState(''); const [filter,setFilter]=useState('Todos'); const [menuOpen,setMenuOpen]=useState(false);
  const sorted = projects.slice().sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));
  const visible=sorted.filter(p=>`${p.name} ${p.client} ${(p.rooms||[]).map(r=>r.name).join(' ')}`.toLowerCase().includes(query.toLowerCase()));
  return <View style={styles.screen}><View style={styles.projectsHeader}><Pressable onPress={()=>setMenuOpen(true)} hitSlop={10} style={styles.projectsMenuBtn}><Text style={styles.projectsMenu}>☰</Text></Pressable><Text style={styles.projectsHeaderTitle}>Meus projetos</Text><Pressable onPress={onNew} style={styles.projectsPlus}><Text style={styles.projectsPlusText}>＋</Text></Pressable></View>
    <View style={styles.searchBox}><Text style={styles.searchIcon}>⌕</Text><TextInput value={query} onChangeText={setQuery} placeholder="Buscar projetos..." placeholderTextColor="#8B94A2" style={styles.searchInput}/></View>
    <View style={styles.filterRow}>{['Todos','Em andamento','Concluídos'].map(f=><Pressable key={f} onPress={()=>setFilter(f)} style={[styles.filterChip,filter===f&&styles.filterChipOn]}><Text style={[styles.filterText,filter===f&&styles.filterTextOn]}>{f}</Text></Pressable>)}</View>
    <ScrollView style={{flex:1}} contentContainerStyle={styles.homeList} showsVerticalScrollIndicator={false}>{visible.length?visible.map((p,i)=><View key={p.id} style={styles.projectCardMock}><Pressable onPress={()=>onOpen(p.id)} style={styles.projectOpenArea}><View style={styles.projectThumb}><View style={styles.thumbWall}/><View style={styles.thumbCabinet}/></View><View style={{flex:1}}><Text style={styles.projectTitleSmall}>{p.name}</Text><Text style={styles.projectMetaSmall}>{p.rooms?.length||0} ambiente{(p.rooms?.length||0)!==1?'s':''} · {new Date(p.updatedAt||Date.now()).toLocaleDateString('pt-BR')}</Text><View style={[styles.statusPill,i%3===1&&styles.statusPillDone]}><Text style={[styles.statusText,i%3===1&&styles.statusTextDone]}>{i%3===1?'Concluído':'Em andamento'}</Text></View></View><Text style={styles.chevSmall}>›</Text></Pressable><Pressable hitSlop={8} onPress={()=>onDelete(p.id)} style={styles.deleteIconBtn}><Text style={styles.deleteIconText}>×</Text></Pressable></View>):<View style={styles.emptyHome}><Text style={styles.emptyHomeIcon}>⌗</Text><Text style={styles.emptyHomeTitle}>Nenhum projeto ainda</Text><Text style={styles.emptyHomeText}>Toque no + para criar sua primeira medição.</Text></View>}</ScrollView>
    <View style={styles.bottomNav}><View style={styles.bottomNavItem}><Text style={styles.bottomNavIconOn}>▣</Text><Text style={styles.bottomNavTextOn}>Projetos</Text></View><Pressable onPress={onClients} style={styles.bottomNavItem}><Text style={styles.bottomNavIcon}>♙</Text><Text style={styles.bottomNavText}>Clientes</Text></Pressable><Pressable onPress={onHelp} style={styles.bottomNavItem}><Text style={styles.bottomNavIcon}>?</Text><Text style={styles.bottomNavText}>Ajuda</Text></Pressable><Pressable onPress={onMore} style={styles.bottomNavItem}><Text style={styles.bottomNavIcon}>•••</Text><Text style={styles.bottomNavText}>Mais</Text></Pressable></View>
    <Modal visible={menuOpen} transparent animationType="fade" onRequestClose={()=>setMenuOpen(false)}><View style={styles.menuBackdrop}><Pressable style={StyleSheet.absoluteFillObject} onPress={()=>setMenuOpen(false)}/><View style={styles.sideMenu}><Text style={styles.sideMenuTitle}>GW Medidas</Text><Text style={styles.sideMenuSub}>Navegação</Text><Pressable style={styles.sideMenuItem} onPress={()=>{setMenuOpen(false);onNew?.()}}><Text style={styles.sideMenuItemText}>＋ Nova medição</Text></Pressable><Pressable style={styles.sideMenuItem} onPress={()=>setMenuOpen(false)}><Text style={styles.sideMenuItemText}>▣ Projetos</Text></Pressable><Pressable style={styles.sideMenuItem} onPress={()=>{setMenuOpen(false);onClients?.()}}><Text style={styles.sideMenuItemText}>♙ Clientes</Text></Pressable><Pressable style={styles.sideMenuItem} onPress={()=>{setMenuOpen(false);onHelp?.()}}><Text style={styles.sideMenuItemText}>? Ajuda</Text></Pressable><Pressable style={styles.sideMenuItem} onPress={()=>{setMenuOpen(false);onMore?.()}}><Text style={styles.sideMenuItemText}>••• Mais</Text></Pressable></View></View></Modal>
  </View>;
}
...
function NumberModal({ visible, title, initial, onCancel, onSave }) {
  const [value,setValue]=useState(numFmt(initial)); useEffect(()=>setValue(numFmt(initial)),[visible,initial]);
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onCancel}><KeyboardAvoidingView style={styles.modalBackdrop} behavior={Platform.OS==='ios'?'padding':undefined}><View style={styles.modalCard}><Text style={styles.modalTitle}>{title}</Text><Text style={styles.label}>Medida em metros</Text><TextInput autoFocus selectTextOnFocus keyboardType="decimal-pad" value={value} onChangeText={setValue} style={styles.bigInput}/><View style={{flexDirection:'row',gap:10}}><Button secondary title="Cancelar" onPress={onCancel}/><Button title="Aplicar" onPress={()=>onSave(clamp(parseMeters(value,initial),.05,20))}/></View></View></KeyboardAvoidingView></Modal>;
}
...
function PlanSymbol({type, selected=false, width=58, swing='left', inward=1}){
  ...
  if(type==='Passagem') return <View style={{width:W,height:30}}><Svg width={W} height={30}>
    <Line x1="1" y1="13" x2="9" y2="13" stroke={TECH.wall} strokeWidth="7"/>
    <Line x1={W-9} y1="13" x2={W-1} y2="13" stroke={TECH.wall} strokeWidth="7"/>
    <Line x1="9" y1="13" x2={W-9} y2="13" stroke={c} strokeWidth="1" strokeDasharray="4 3"/>
    <SvgText x={W/2} y="28" textAnchor="middle" fontSize="6.8" fill={TECH.dim}>PASSAGEM</SvgText>
  </Svg></View>;
...
function PlanEquipmentElement({element, wall, wallLength, wallCount, zoom, selected, onSelect, onChange, pinchingRef, canvasW, canvasH, planPpm}){
  ...
  const depthPx=clamp((element.depth||.55)*ppm,18,72),visualW=clamp(element.width*ppm,28,140);
...
function PlanEditor({ project, room, draftMeta, onBack, onSave, onSaveExit, onChooseCount, onPhotos, onFinish, onExportPreview }) {
  ...
  const updateLength=(i,v)=>{const lengths=[...room.lengths];lengths[i]=v;onSave({...room,lengths},false);setEditWall(null)};
  ...
  const addElement=type=>{const d=DEFAULTS[type]||{width:.2,height:.2,bottom:0};const wallW=room.lengths[selected]||3;const free=isFreePlanType(type);const el={id:uid(),type,wall:selected,width:d.width,height:d.height,bottom:d.bottom,depth:d.depth||.15,thickness:d.thickness,swing:d.swing||undefined,left:clamp((wallW-d.width)/2,0,wallW),...(free?{free:true,freeX:.50,freeY:.58}:{})};onSave({...room,elements:[...(room.elements||[]),el]},false);setSelectedElement(el.id);setCanvasFree(false);setAddOpen(false)};
  const updateElement=o=>onSave({...room,elements:(room.elements||[]).map(e=>e.id===o.id?o:e)},false);
  const deleteSelected=()=>{if(!selectedElement)return;Alert.alert('Excluir item','Remover este item da medição?',[{text:'Cancelar',style:'cancel'},{text:'Excluir',style:'destructive',onPress:()=>{onSave({...room,elements:(room.elements||[]).filter(e=>e.id!==selectedElement)},false);setSelectedElement(null);setEditElementOpen(false)}}])};
  ...
  const frontW=room.lengths[selected]||3.2, frontH=room.height||2.65, frontPx=Math.min((canvasW-54)/frontW,(canvasH-56)/frontH), frontObjects=(room.elements||[]).filter(e=>e.wall===selected&&!isFreePlanType(e.type)&&e.free!==true);
...
function PlanPreviewStatic({room,width=360,height=230}){
  ...
  return <View style={[styles.planCanvas,{width,height,overflow:'hidden'}]} pointerEvents="none">
    ...
    <View pointerEvents="none" style={{flex:1}}>
      ...
      {(room.elements||[]).filter(e=>isEquipment(e.type)||isReservedSpace(e.type)).map(e=>{
        const free=isFreePlanType(e.type)||e.free===true,w=walls[e.wall]||walls[0];
        return w?<PlanEquipmentElement key={`preview-eq-${e.id}`} element={e} wall={w} wallLength={room.lengths[e.wall]||room.lengths[0]||1} wallCount={room.wallCount||4} zoom={1} selected={false} onSelect={noop} onChange={noop} canvasW={width} canvasH={height} planPpm={planPpm}/>:null;
      })}
    </View>
  </View>;
}
function FrontPreviewStatic({room,wallIndex=0,width=360,height=230}){
  const wallW=room.lengths[wallIndex]||3.2,wallH=room.height||2.65,px=Math.min((width-60)/wallW,(height-60)/wallH), objects=(room.elements||[]).filter(e=>e.wall===wallIndex);
  return <View style={{width,height,overflow:'hidden',borderRadius:12}}><TechnicalElevationBase wallW={wallW} wallH={wallH} px={px} width={width} height={height}/>{objects.map(o=>{const opening=isOpening(o.type),equip=isEquipment(o.type),reserved=isReservedSpace(o.type),structure=isStructure(o.type),point=isPoint(o.type),vw=Math.max(o.width*px,opening||equip||reserved?28:18),vh=o.type==='Pia'?Math.max(o.height*px,12):Math.max(o.height*px,opening||equip||reserved?26:18),x=27+o.left*px,y=46+(wallH-o.bottom-o.height)*px;return <View key={o.id} pointerEvents="none" style={{position:'absolute',left:x,top:y,width:vw,height:vh,borderWidth:1,borderColor:'#5B6670',backgroundColor:point?'transparent':'rgba(255,255,255,.2)'}}><ElementVisual type={o.type} width={vw} height={vh}/></View>})}</View>;
}
...
function ObjectModal({ visible, object, wallW, wallH, onClose, onSave, onDelete }) {
  ...
  useSpeechRecognitionEvent('start',()=>{if(visible&&object){setVoiceListening(true);setVoiceMessage('Ouvindo… fale as medidas.')}});
  useSpeechRecognitionEvent('end',()=>{if(visible&&object)setVoiceListening(false)});
  useSpeechRecognitionEvent('result',event=>{if(!visible||!object)return;const t=event.results?.[0]?.transcript||'';if(t)setVoiceText(t);if(event.isFinal&&t)applyVoiceMeasures(t);});
  useSpeechRecognitionEvent('error',event=>{if(!visible||!object)return;setVoiceListening(false);if(event.error!=='aborted')setVoiceMessage('Não consegui ouvir. Toque no microfone e tente novamente.');});
  ...
  const toggleVoice=async()=>{try{if(voiceListening){ExpoSpeechRecognitionModule.stop();return;}const permission=await ExpoSpeechRecognitionModule.requestPermissionsAsync();if(!permission.granted){Alert.alert('Microfone','Autorize Microfone e Reconhecimento de Fala para ditar as medidas.');return;}setVoiceText('');setVoiceMessage('');ExpoSpeechRecognitionModule.start({lang:'pt-BR',interimResults:true,continuous:false,maxAlternatives:1,addsPunctuation:false});}catch(e){console.warn('voice',e);Alert.alert('Voz','O reconhecimento de voz precisa do GW Medidas Development Build. Ele não funciona dentro do Expo Go.');}};
...
function Elevation({ project, room, wallIndex, onBack, onUpdateRoom }) {
  ...
  const objects=(room.elements||[]).filter(e=>e.wall===wallIndex), selected=objects.find(e=>e.id===selectedId);
...
const reportPlanSvg=room=>{
  ...
  const elems=ordered.map(e=>{
    const free=isFreePlanType(e.type)||e.free===true,w=walls[e.wall]||walls[0];if(!w)return'';const [a,b]=w,dx=b[0]-a[0],dy=b[1]-a[1],len=Math.hypot(dx,dy)||1,ux=dx/len,uy=dy/len,iv=inward(a,b),nx=iv[0],ny=iv[1];
...
const reportFrontSvg=(room,i=0)=>{
  const wallW=(room.lengths||[])[i]||3.2,W=560,H=330,x=45,y=48,wallH=room.height||2.65,px=Math.min((W-100)/wallW,(H-90)/wallH),ww=wallW*px,hh=wallH*px,objects=(room.elements||[]).filter(e=>e.wall===i);
  const objs=objects.map((e,k)=>{const ex=x+e.left*px,ey=y+(wallH-e.bottom-e.height)*px,ew=Math.max(5,e.width*px),eh=Math.max(5,e.height*px),fill=e.type==='Porta'?'#c99a68':e.type==='Janela'?'#cdeaf5':isEquipment(e.type)?'#dfe3e5':isStructure(e.type)?'#cbd1d4':isReservedSpace(e.type)?'#ffffff':'#fff';return `<g><rect x="${ex}" y="${ey}" width="${ew}" height="${eh}" fill="${fill}" stroke="#58636a" stroke-width="1.1"/>${e.type==='Porta'?`<circle cx="${ex+ew*.83}" cy="${ey+eh*.52}" r="2" fill="#30363a"/>`:''}${e.type==='Janela'?`<line x1="${ex+ew/2}" y1="${ey}" x2="${ex+ew/2}" y2="${ey+eh}" stroke="#6b808a"/><line x1="${ex}" y1="${ey+eh/2}" x2="${ex+ew}" y2="${ey+eh/2}" stroke="#6b808a"/>`:''}<line x1="${ex}" y1="${ey+eh+8+k%3*4}" x2="${ex+ew}" y2="${ey+eh+8+k%3*4}" stroke="#64717b" stroke-width=".7"/><text x="${ex+ew/2}" y="${ey+eh+6+k%3*4}" font-size="6.7" text-anchor="middle" fill="#46515b">${numFmt(e.width)}</text></g>`}).join('');
...
function ClientsScreen({projects,onBack}){
  const clients=useMemo(()=>{const m=new Map();projects.forEach(p=>{const k=(p.client||'Sem cliente').trim()||'Sem cliente',cur=m.get(k)||{name:k,projects:0,rooms:0};cur.projects+=1;cur.rooms+=(p.rooms||[]).length;m.set(k,cur)});return [...m.values()].sort((a,b)=>a.name.localeCompare(b.name,'pt-BR'))},[projects]);
  return <View style={styles.screen}><Header title="Clientes" subtitle={`${clients.length} cliente${clients.length!==1?'s':''}`} onBack={onBack}/><ScrollView contentContainerStyle={styles.simplePage}>{clients.length?clients.map(c=><View key={c.name} style={styles.infoCard}><View style={styles.infoIcon}><Text style={styles.infoIconText}>{c.name.slice(0,1).toUpperCase()}</Text></View><View style={{flex:1}}><Text style={styles.infoTitle}>{c.name}</Text><Text style={styles.infoText}>{c.projects} projeto{c.projects!==1?'s':''} · {c.rooms} ambiente{c.rooms!==1?'s':''}</Text></View></View>):<Text style={styles.emptyHomeText}>Os clientes aparecerão aqui conforme você criar projetos.</Text>}</ScrollView></View>
}
...
function MoreScreen({onBack,onNew}){
  return <View style={styles.screen}><Header title="Mais" subtitle="GW Medidas" onBack={onBack}/><ScrollView contentContainerStyle={styles.simplePage}><Pressable onPress={onNew} style={styles.moreAction}><Text style={styles.moreActionTitle}>＋ Nova medição</Text><Text style={styles.moreActionText}>Criar um novo ambiente ou projeto.</Text></Pressable><View style={styles.infoCard}><View style={{flex:1}}><Text style={styles.infoTitle}>GW Medidas</Text><Text style={styles.infoText}>Medição técnica para marcenaria · versão 6.2.0</Text></View></View><View style={styles.infoCard}><View style={{flex:1}}><Text style={styles.infoTitle}>Dados</Text><Text style={styles.infoText}>Nesta fase, seus projetos continuam salvos localmente no aparelho.</Text></View></View></ScrollView></View>
}

export default function App(){
  const [ready,setReady]=useState(false),[screen,setScreen]=useState('welcome'),[projects,setProjects]=useState([]),[activeProjectId,setActiveProjectId]=useState(null),[activeRoomId,setActiveRoomId]=useState(null),[draftMeta,setDraftMeta]=useState(null);
  useEffect(()=>{AsyncStorage.getItem(STORE_KEY).then(v=>{if(v)try{setProjects(JSON.parse(v))}catch{}setReady(true)})},[]);
  ...
  const deleteProject=id=>Alert.alert('Excluir projeto','Excluir este projeto e todos os ambientes?',[{text:'Cancelar',style:'cancel'},{text:'Excluir',style:'destructive',onPress:async()=>{await persist(projects.filter(p=>p.id!==id));if(activeProjectId===id){setActiveProjectId(null);setActiveRoomId(null);setScreen('home')}}}]);
  const deleteRoom=id=>Alert.alert('Excluir ambiente','Excluir este ambiente e todas as medidas dele?',[{text:'Cancelar',style:'cancel'},{text:'Excluir',style:'destructive',onPress:async()=>{const next=projects.map(p=>p.id!==activeProjectId?p:{...p,updatedAt:Date.now(),rooms:p.rooms.filter(r=>r.id!==id)});await persist(next);if(activeRoomId===id)setActiveRoomId(null)}}]);
  ...
  else if(screen==='home')content=<Home projects={projects} onNew={()=>setScreen('new')} onOpen={id=>{setActiveProjectId(id);setScreen('project')}} onDelete={deleteProject} onClients={()=>setScreen('clients')} onHelp={()=>setScreen('help')} onMore={()=>setScreen('more')}/>;
  else if(screen==='clients')content=<ClientsScreen projects={projects} onBack={()=>setScreen('home')}/>;
  else if(screen==='help')content=<HelpScreen onBack={()=>setScreen('home')}/>;
  else if(screen==='more')content=<MoreScreen onBack={()=>setScreen('home')} onNew={()=>setScreen('new')}/>;
  else if(screen==='new')content=<NewMeasurement projects={projects} onBack={()=>setScreen('home')} onContinue={begin}/>;
  else if(screen==='project')content=<ProjectScreen project={project} onBack={()=>setScreen('home')} onOpenRoom={id=>{setActiveRoomId(id);setScreen('measure')}} onNewRoom={()=>newRoomForProject(project)} onDeleteRoom={deleteRoom} onDeleteProject={()=>deleteProject(project.id)}/>;
  ...
  return <GestureHandlerRootView style={{flex:1}}><AppFrame>{content}</AppFrame></GestureHandlerRootView>;
}

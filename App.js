...
// GW Medidas 6.6.3 — editor profissional da medição rápida
...
function QuickPhotoMeasure({job,onBack,onUpdate,onSave,onFinish}){
  ...
  const resetView=()=>{setZoom(1);setOffset({x:0,y:0});setTool('navigate')};
  ...
  const drawPan=useMemo(()=>Gesture.Pan().minDistance(4).runOnJS(true).onBegin(e=>{if(tool==='measure'){const q=localPoint(e);setDraftLine({x1:q.x,y1:q.y,x2:q.x,y2:q.y})}else if(tool==='area'){const q=localPoint(e);setDraftArea({x1:q.x,y1:q.y,x2:q.x,y2:q.y})}else if(tool==='navigate'){panStartRef.current=offsetRef.current}}).onUpdate(e=>{if(tool==='measure'){const q=localPoint(e);setDraftLine(d=>d?{...d,x2:q.x,y2:q.y}:d)}else if(tool==='area'){const q=localPoint(e);setDraftArea(d=>d?{...d,x2:q.x,y2:q.y}:d)}else if(tool==='navigate'&&zoomRef.current>1){setOffset({x:panStartRef.current.x+e.translationX,y:panStartRef.current.y+e.translationY})}}).onEnd(e=>{if(tool==='measure'){setDraftLine(d=>{if(!d)return null;const q=localPoint(e),line={...d,x2:q.x,y2:q.y};if(Math.hypot(line.x2-line.x1,line.y2-line.y1)>.025){setPendingLine(line);setMeasureValue('')}return null})}else if(tool==='area'){setDraftArea(d=>{if(!d)return null;const q=localPoint(e),a={...d,x2:q.x,y2:q.y};if(Math.abs(a.x2-a.x1)>.025&&Math.abs(a.y2-a.y1)>.025){setPendingArea(a);setAreaValue('')}return null})}}),[tool,boxW,boxH]);
  ...
  const photoGesture=useMemo(()=>Gesture.Simultaneous(drawPan,pinch),[drawPan,pinch]);
  const confirmMeasure=()=>{if(!activePhoto||!pendingLine)return;const value=measureValue.trim();if(!value)return;const mark={id:uid(),kind:'line',...pendingLine,value};const updated={...activePhoto,marks:[...(activePhoto.marks||[]),mark]};onUpdate({...job,photos:(job.photos||[]).map(p=>p.id===updated.id?updated:p),updatedAt:Date.now()});setActivePhoto(updated);setPendingLine(null);setMeasureValue('');setTool('navigate')};
  const confirmArea=()=>{if(!activePhoto||!pendingArea)return;const value=areaValue.trim();if(!value)return;const mark={id:uid(),kind:'area',...pendingArea,value};const updated={...activePhoto,marks:[...(activePhoto.marks||[]),mark]};onUpdate({...job,photos:(job.photos||[]).map(p=>p.id===updated.id?updated:p),updatedAt:Date.now()});setActivePhoto(updated);setPendingArea(null);setAreaValue('');setTool('navigate')};
  ...
  return <View style={styles.screen}><Header title="Medição rápida" subtitle={`${job.client} · ${job.project}`} onBack={onBack}/><ScrollView scrollEnabled={false} contentContainerStyle={{paddingBottom:148}}><View style={styles.quickToolHeader}><View><Text style={styles.quickToolHeaderTitle}>{tool==='navigate'?'Foto pronta para medir':tool==='measure'?'Ferramenta Medida':tool==='area'?'Ferramenta Área':'Anotação'}</Text><Text style={styles.quickToolHeaderSub}>{tool==='navigate'?'Arraste para navegar · pinça para zoom':tool==='measure'?'Arraste entre os dois pontos da medida':tool==='area'?'Arraste para delimitar a área':'Registre uma observação desta foto'}</Text></View>{zoom>1?<Pressable onPress={resetView} style={styles.quickResetZoom}><Text style={styles.quickResetZoomText}>100%</Text></Pressable>:null}</View>
    <View style={[styles.quickPhotoStage,{width:boxW,height:boxH}]}><GestureDetector gesture={photoGesture}><View style={StyleSheet.absoluteFillObject}><View style={[StyleSheet.absoluteFillObject,{transform:[{translateX:offset.x},{translateY:offset.y},{scale:zoom}]}]}><Image source={{uri:activePhoto.uri}} style={StyleSheet.absoluteFillObject} resizeMode="contain"/><Svg width={boxW} height={boxH}>{marks.map(m=>{if(m.kind==='area'){const x=Math.min(m.x1,m.x2)*boxW,y=Math.min(m.y1,m.y2)*boxH,w=Math.abs(m.x2-m.x1)*boxW,h=Math.abs(m.y2-m.y1)*boxH;return <React.Fragment key={m.id}><Rect x={x} y={y} width={w} height={h} rx="4" fill="rgba(22,119,242,.16)" stroke={BLUE} strokeWidth="2"/><Rect x={x+w/2-34} y={y+h/2-13} width="68" height="26" rx="7" fill={WHITE} stroke={BLUE}/><SvgText x={x+w/2} y={y+h/2+5} fontSize="11" fontWeight="800" textAnchor="middle" fill={INK}>{m.value}</SvgText></React.Fragment>}const x1=m.x1*boxW,y1=m.y1*boxH,x2=m.x2*boxW,y2=m.y2*boxH,mx=(x1+x2)/2,my=(y1+y2)/2;return <React.Fragment key={m.id}><Line x1={x1} y1={y1} x2={x2} y2={y2} stroke={BLUE} strokeWidth="2.5"/><Circle cx={x1} cy={y1} r="5" fill={WHITE} stroke={BLUE} strokeWidth="2.5"/><Circle cx={x2} cy={y2} r="5" fill={WHITE} stroke={BLUE} strokeWidth="2.5"/><Rect x={mx-36} y={my-13} width="72" height="26" rx="7" fill={WHITE} stroke={BLUE}/><SvgText x={mx} y={my+5} fontSize="12" fontWeight="800" textAnchor="middle" fill={INK}>{m.value}</SvgText></React.Fragment>})}{draftLine?(()=>{const x1=draftLine.x1*boxW,y1=draftLine.y1*boxH,x2=draftLine.x2*boxW,y2=draftLine.y2*boxH;return <><Line x1={x1} y1={y1} x2={x2} y2={y2} stroke={BLUE} strokeWidth="2.5"/><Circle cx={x1} cy={y1} r="5" fill={WHITE} stroke={BLUE}/><Circle cx={x2} cy={y2} r="5" fill={WHITE} stroke={BLUE}/></>})():null}{draftArea?(()=>{const x=Math.min(draftArea.x1,draftArea.x2)*boxW,y=Math.min(draftArea.y1,draftArea.y2)*boxH,w=Math.abs(draftArea.x2-draftArea.x1)*boxW,h=Math.abs(draftArea.y2-draftArea.y1)*boxH;return <Rect x={x} y={y} width={w} height={h} fill="rgba(22,119,242,.14)" stroke={BLUE} strokeWidth="2"/>})():null}</Svg></View></View></GestureDetector></View>
    {marks.length?<View style={styles.quickMarksList}>{marks.map((m,i)=><View key={m.id} style={styles.quickMarkRow}><Text style={styles.quickMarkIndex}>{i+1}</Text><Text style={styles.quickMarkValue}>{m.kind==='area'?'Área · ':''}{m.value}</Text><Pressable onPress={()=>deleteMark(m.id)}><Text style={styles.quickMarkDelete}>Excluir</Text></Pressable></View>)}</View>:null}

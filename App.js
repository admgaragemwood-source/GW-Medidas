...
function ProjectsLibrary({projects,quickJobs=[],onBack,onHome,onOpen,onOpenQuick,onDelete,onDeleteQuick,onClients,onMore}){
...
const reportPlanSvg=room=>{
  ...
  const dimLine=(a,b,i)=>{
    const dx=b[0]-a[0],dy=b[1]-a[1],len=Math.hypot(dx,dy)||1,ux=dx/len,uy=dy/len,iv=inward(a,b),nx=-iv[0],ny=-iv[1],off=24;
    const ax=a[0]+nx*off,ay=a[1]+ny*off,bx=b[0]+nx*off,by=b[1]+ny*off,mx=(ax+bx)/2,my=(ay+by)/2;
    const wallLength=Number(ls[i]||0);
    const relevant=(room.elements||[]).filter(e=>elementWallIndex(e,room)===i&&!isFreePlanType(e.type)&&e.free!==true);
    const cuts=[0,wallLength]; relevant.forEach(e=>{const aw=alongWallWidth(e);cuts.push(clamp(Number(e.left||0),0,wallLength));cuts.push(clamp(Number(e.left||0)+aw,0,wallLength))});
    const vals=[...new Set(cuts.map(v=>Math.round(v*1000)/1000))].sort((p,q)=>p-q);
    const segments=vals.slice(0,-1).map((v,j)=>{
      const v2=vals[j+1],seg=v2-v;if(seg<.08)return '';
      const p1=[a[0]+ux*(v/wallLength)*len,a[1]+uy*(v/wallLength)*len],p2=[a[0]+ux*(v2/wallLength)*len,a[1]+uy*(v2/wallLength)*len];
      const tier=((Math.hypot(p2[0]-p1[0],p2[1]-p1[1])<44)?j%2:0),soff=42+tier*13;
      const s1=[p1[0]+nx*soff,p1[1]+ny*soff],s2=[p2[0]+nx*soff,p2[1]+ny*soff],sm=[(s1[0]+s2[0])/2,(s1[1]+s2[1])/2];
      return `<line x1="${p1[0]+nx*7}" y1="${p1[1]+ny*7}" x2="${s1[0]}" y2="${s1[1]}" stroke="#a6b3be" stroke-width=".65"/><line x1="${p2[0]+nx*7}" y1="${p2[1]+ny*7}" x2="${s2[0]}" y2="${s2[1]}" stroke="#a6b3be" stroke-width=".65"/><line x1="${s1[0]}" y1="${s1[1]}" x2="${s2[0]}" y2="${s2[1]}" stroke="#7f91a1" stroke-width=".8"/><rect x="${sm[0]-17}" y="${sm[1]-6}" width="34" height="12" rx="4" fill="#fff"/><text x="${sm[0]}" y="${sm[1]+2.5}" font-size="7.2" font-weight="700" text-anchor="middle" fill="#536473">${numFmt(seg)}</text>`;
    }).join('');
    return `<line x1="${a[0]}" y1="${a[1]}" x2="${ax}" y2="${ay}" stroke="#9aa8b5" stroke-width=".7"/><line x1="${b[0]}" y1="${b[1]}" x2="${bx}" y2="${by}" stroke="#9aa8b5" stroke-width=".7"/><line x1="${ax}" y1="${ay}" x2="${bx}" y2="${by}" stroke="#485968" stroke-width="1"/><rect x="${mx-28}" y="${my-9}" width="56" height="16" rx="5" fill="#fff" stroke="#e2e8ee"/><text x="${mx}" y="${my+2}" font-size="9" font-weight="700" text-anchor="middle" fill="#46515c">${numFmt(ls[i])} m</text>${segments}`;
  };
  ...
  const elems=ordered.map(e=>{
    ...
    const x1=a[0]+ux*(e.left*scale),y1=a[1]+uy*(e.left*scale),x2=a[0]+ux*((e.left+alongWallWidth(e))*scale),y2=a[1]+uy*((e.left+alongWallWidth(e))*scale);
...
const reportFrontSvg=(room,i=0)=>{
  const wallW=(room.lengths||[])[i]||3.2,W=560,H=330,x=45,y=58,wallH=room.height||2.65,px=Math.min((W-100)/wallW,(H-105)/wallH),ww=wallW*px,hh=wallH*px;
  const objects=(room.elements||[]).filter(e=>elementWallIndex(e,room)===i&&(!isFreePlanType(e.type)||isInternalWall(e.type))&&(e.free!==true||isInternalWall(e.type))).map(e=>isInternalWall(e.type)?frontObjectForRoom(e,i,room):constrainElementToWallSegments(room,e)).sort((a,b)=>visualLayer(a)-visualLayer(b));
  const cuts=[0,wallW];objects.filter(e=>isOpening(e.type)||isEquipment(e.type)||isStructure(e.type)||isReservedSpace(e.type)).forEach(e=>{cuts.push(clamp(Number(e.left||0),0,wallW));cuts.push(clamp(Number(e.left||0)+Number(e.width||0),0,wallW))});
  const vals=[...new Set(cuts.map(v=>Math.round(v*1000)/1000))].sort((a,b)=>a-b);
  const segs=vals.slice(0,-1).map((v,j)=>{const v2=vals[j+1],seg=v2-v;if(seg<.08)return'';const x1=x+v*px,x2=x+v2*px,c=(x1+x2)/2,tier=((x2-x1)<44)?j%2:0,yy=14-tier*13;return `<line x1="${x1}" y1="${y}" x2="${x1}" y2="${yy}" stroke="#a6b3be" stroke-width=".65"/><line x1="${x2}" y1="${y}" x2="${x2}" y2="${yy}" stroke="#a6b3be" stroke-width=".65"/><line x1="${x1}" y1="${yy}" x2="${x2}" y2="${yy}" stroke="#718496" stroke-width=".8"/><rect x="${c-15}" y="${yy-6}" width="30" height="12" rx="4" fill="#fff"/><text x="${c}" y="${yy+2.5}" font-size="6.8" font-weight="700" text-anchor="middle" fill="#4a5a68">${numFmt(seg)}</text>`}).join('');
  const objs=objects.map(e=>{
    const ex=x+e.left*px,ey=y+(wallH-e.bottom-e.height)*px;
    const point=isPoint(e.type),internal=isInternalWall(e.type);
    const ew=point?18:Math.max(internal?5:5,e.width*px),eh=point?18:Math.max(5,e.height*px);
    const cx=ex+ew/2,cy=ey+eh/2;
    if(internal)return `<g><rect x="${ex}" y="${ey}" width="${ew}" height="${eh}" fill="#D4D9DD" stroke="#59636B" stroke-width="1.5"/><line x1="${ex+ew*.35}" y1="${ey}" x2="${ex+ew*.35}" y2="${ey+eh}" stroke="#B5BDC2" stroke-width=".7"/></g>`;
    if(e.type==='Porta')return `<g><defs><linearGradient id="door-${e.id}" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#D0A777"/><stop offset="1" stop-color="#B98A59"/></linearGradient></defs><rect x="${ex}" y="${ey}" width="${ew}" height="${eh}" fill="#6D5137" stroke="#59422F" stroke-width="1.4"/><rect x="${ex+ew*.07}" y="${ey+eh*.03}" width="${ew*.86}" height="${eh*.94}" fill="url(#door-${e.id})" stroke="#59422F" stroke-width="1"/><rect x="${ex+ew*.17}" y="${ey+eh*.14}" width="${ew*.66}" height="${eh*.30}" fill="none" stroke="#8C6845" stroke-width="1.2"/><rect x="${ex+ew*.17}" y="${ey+eh*.54}" width="${ew*.66}" height="${eh*.34}" fill="none" stroke="#8C6845" stroke-width="1.2"/><circle cx="${ex+ew*.80}" cy="${ey+eh*.51}" r="${Math.max(1.4,Math.min(2.4,ew*.04))}" fill="#31363A"/></g>`;
    if(e.type==='Janela')return `<g><rect x="${ex}" y="${ey}" width="${ew}" height="${eh}" fill="#67747C" stroke="#4D606A" stroke-width="1.2"/><rect x="${ex+ew*.07}" y="${ey+eh*.07}" width="${ew*.86}" height="${eh*.82}" fill="#C7E8F3" stroke="#4D606A" stroke-width=".9"/><line x1="${cx}" y1="${ey+eh*.07}" x2="${cx}" y2="${ey+eh*.89}" stroke="#5D7079" stroke-width="1.2"/><line x1="${ex+ew*.07}" y1="${cy}" x2="${ex+ew*.93}" y2="${cy}" stroke="#6D8089" stroke-width=".9"/><rect x="${ex+ew*.03}" y="${ey+eh*.89}" width="${ew*.94}" height="${eh*.08}" fill="#AEB7BC"/></g>`;
    if(e.type==='Pia')return `<g><defs><linearGradient id="sink-${e.id}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#F3F4F4"/><stop offset="1" stop-color="#B8C0C4"/></linearGradient></defs><rect x="${ex}" y="${ey+eh*.34}" width="${ew}" height="${eh*.34}" rx="2" fill="url(#sink-${e.id})" stroke="#4E565C" stroke-width="1.2"/><rect x="${ex}" y="${ey+eh*.62}" width="${ew}" height="${eh*.10}" fill="#8E979C"/><rect x="${ex+ew*.31}" y="${ey+eh*.39}" width="${ew*.38}" height="${eh*.20}" rx="2" fill="#C3D0D5" stroke="#6E8089" stroke-width=".7"/><path d="M ${cx} ${ey+eh*.38} q 0 ${-Math.max(4,eh*.18)} ${Math.max(5,ew*.12)} ${-Math.max(4,eh*.18)}" fill="none" stroke="#596A73" stroke-width="1.2"/></g>`;
    if(e.type==='Geladeira')return `<g><defs><linearGradient id="fr-${e.id}" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#FAFBFC"/><stop offset="1" stop-color="#C8CED2"/></linearGradient></defs><rect x="${ex}" y="${ey}" width="${ew}" height="${eh}" rx="3" fill="url(#fr-${e.id})" stroke="#5A6268" stroke-width="1.2"/><line x1="${ex}" y1="${ey+eh*.38}" x2="${ex+ew}" y2="${ey+eh*.38}" stroke="#7D868C"/><line x1="${ex+ew*.82}" y1="${ey+eh*.11}" x2="${ex+ew*.82}" y2="${ey+eh*.31}" stroke="#626B71" stroke-width="1.5"/><line x1="${ex+ew*.82}" y1="${ey+eh*.47}" x2="${ex+ew*.82}" y2="${ey+eh*.70}" stroke="#626B71" stroke-width="1.5"/></g>`;
    if(e.type==='Fogão')return `<g><rect x="${ex}" y="${ey}" width="${ew}" height="${eh}" rx="2" fill="#5C646A" stroke="#22282C" stroke-width="1.2"/><rect x="${ex+ew*.08}" y="${ey+eh*.08}" width="${ew*.84}" height="${eh*.18}" fill="#32383D"/><rect x="${ex+ew*.13}" y="${ey+eh*.38}" width="${ew*.74}" height="${eh*.44}" rx="2" fill="#20262A" stroke="#9AA1A5"/><rect x="${ex+ew*.20}" y="${ey+eh*.45}" width="${ew*.60}" height="${eh*.28}" fill="#56636B"/></g>`;
    if(e.type==='Cooktop')return `<g><rect x="${ex}" y="${ey+eh*.25}" width="${ew}" height="${eh*.50}" rx="2" fill="#30363A" stroke="#161B1E" stroke-width="1.1"/>${[[.25,.4],[.5,.4],[.75,.4],[.25,.62],[.5,.62],[.75,.62]].map(p=>`<circle cx="${ex+ew*p[0]}" cy="${ey+eh*p[1]}" r="${Math.max(1.2,Math.min(3,ew*.07))}" fill="#111719" stroke="#A4AAAD" stroke-width=".6"/>`).join('')}</g>`;
    if(e.type==='Máquina de lavar'||e.type==='Lava-louças')return `<g><rect x="${ex}" y="${ey}" width="${ew}" height="${eh}" rx="3" fill="#E9ECEE" stroke="#59656C" stroke-width="1.2"/><rect x="${ex+ew*.08}" y="${ey+eh*.08}" width="${ew*.84}" height="${eh*.15}" fill="#D4D9DC"/>${e.type==='Máquina de lavar'?`<circle cx="${cx}" cy="${ey+eh*.60}" r="${Math.min(ew,eh)*.27}" fill="#9EB5C0" stroke="#5B6A72" stroke-width="1.2"/><circle cx="${cx}" cy="${ey+eh*.60}" r="${Math.min(ew,eh)*.19}" fill="#C6DBE3" stroke="#7A8C94" stroke-width=".8"/>`:`<line x1="${ex+ew*.14}" y1="${ey+eh*.29}" x2="${ex+ew*.86}" y2="${ey+eh*.29}" stroke="#7B858B"/><rect x="${ex+ew*.15}" y="${ey+eh*.39}" width="${ew*.70}" height="${eh*.45}" fill="#D7DCDF" stroke="#879096" stroke-width=".7"/>`}</g>`;
    if(e.type==='Tomada')return `<g><rect x="${ex+2}" y="${ey+3}" width="${ew-4}" height="${eh-6}" rx="3" fill="#FBFCFD" stroke="#46515A" stroke-width="1.2"/><circle cx="${cx-3}" cy="${cy-1}" r="1.3" fill="#46515A"/><circle cx="${cx+3}" cy="${cy-1}" r="1.3" fill="#46515A"/><path d="M ${cx} ${cy+2} l -2 3 h 4 Z" fill="#46515A"/></g>`;
    if(e.type==='Interruptor')return `<g><rect x="${ex+3}" y="${ey+1}" width="${ew-6}" height="${eh-2}" rx="3" fill="#FBFCFD" stroke="#46515A" stroke-width="1.2"/><rect x="${ex+ew*.34}" y="${ey+eh*.24}" width="${ew*.32}" height="${eh*.52}" rx="1.5" fill="#E8EDF0" stroke="#6A757D" stroke-width=".7"/></g>`;
    if(e.type==='Água')return `<circle cx="${cx}" cy="${cy}" r="6" fill="#69BFE7" stroke="#39738C" stroke-width="1"/>`;
    if(e.type==='Esgoto')return `<circle cx="${cx}" cy="${cy}" r="6" fill="#30383E" stroke="#111820" stroke-width="1"/>`;
    if(e.type==='Gás')return `<circle cx="${cx}" cy="${cy}" r="6" fill="#E6B84A" stroke="#75602D" stroke-width="1"/>`;
    if(isReservedSpace(e.type))return `<g><rect x="${ex}" y="${ey}" width="${ew}" height="${eh}" rx="2" fill="rgba(255,255,255,.55)" stroke="#5B6670" stroke-width="1.1" stroke-dasharray="5 3"/><line x1="${ex+5}" y1="${cy}" x2="${ex+ew-5}" y2="${cy}" stroke="#8A98A3" stroke-width=".7" stroke-dasharray="3 3"/></g>`;
    const fill=isStructure(e.type)?'#CBD1D4':isEquipment(e.type)?'#E9ECEE':'#FFFFFF';
    return `<g><rect x="${ex}" y="${ey}" width="${ew}" height="${eh}" rx="2" fill="${fill}" stroke="#58636A" stroke-width="1.1"/><line x1="${ex+4}" y1="${cy}" x2="${ex+ew-4}" y2="${cy}" stroke="#87949D" stroke-width=".6"/></g>`;
  }).join('');
  return `<svg viewBox="0 0 ${W} ${H}" width="100%" height="255" xmlns="http://www.w3.org/2000/svg"><rect width="100%" height="100%" fill="#eef2f5"/>${segs}<rect x="${x+4}" y="${y+5}" width="${ww}" height="${hh}" fill="#c4cbd0" opacity=".3"/><rect x="${x}" y="${y}" width="${ww}" height="${hh}" fill="#faf8f4" stroke="#30363a" stroke-width="2.2"/><rect x="${x}" y="${y+hh}" width="${ww}" height="22" fill="#ddd4c8"/><line x1="${x}" y1="${y-18}" x2="${x+ww}" y2="${y-18}" stroke="#586572" stroke-width=".8"/><rect x="${x+ww/2-28}" y="${y-27}" width="56" height="16" rx="5" fill="#fff" stroke="#e2e8ee"/><text x="${x+ww/2}" y="${y-16}" font-size="9" font-weight="700" text-anchor="middle" fill="#46515c">${numFmt(wallW)} m</text><line x1="${x+ww+18}" y1="${y}" x2="${x+ww+18}" y2="${y+hh}" stroke="#586572" stroke-width=".8"/><text x="${x+ww+29}" y="${y+hh/2}" font-size="8" fill="#46515c" transform="rotate(90 ${x+ww+29} ${y+hh/2})">${numFmt(wallH)} m</text>${objs}</svg>`;
};
...
const svgMarkupToPngDataUrl=async(svgMarkup,width=1120)=>{
  if(typeof document==='undefined')return '';
  return await new Promise((resolve,reject)=>{
    try{
      const blob=new Blob([svgMarkup],{type:'image/svg+xml;charset=utf-8'}),url=URL.createObjectURL(blob),img=new window.Image();
      img.onload=()=>{try{const vb=(svgMarkup.match(/viewBox=[\"']0 0 ([\d.]+) ([\d.]+)[\"']/)||[]),sw=Number(vb[1]||560),sh=Number(vb[2]||390),h=Math.round(width*(sh/sw)),canvas=document.createElement('canvas');canvas.width=width;canvas.height=h;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,width,h);ctx.drawImage(img,0,0,width,h);URL.revokeObjectURL(url);resolve(canvas.toDataURL('image/png'));}catch(e){URL.revokeObjectURL(url);reject(e)}};
      img.onerror=e=>{URL.revokeObjectURL(url);reject(e)};img.src=url;
    }catch(e){reject(e)}
  });
};
const projectFourPagePdfHtml=async(project,rooms,wallsByRoom={})=>{
  const pages=[];
  for(let ri=0;ri<rooms.length;ri++){
    const room=rooms[ri], chosenWall=Number.isInteger(wallsByRoom?.[room.id])?wallsByRoom[room.id]:0;
    const plan=await svgMarkupToPngDataUrl(reportPlanSvg(room));
    const front=await svgMarkupToPngDataUrl(reportFrontSvg(room,chosenWall));
    const head=(label)=>`<div class="top"><div><div class="brand">GW MEDIDAS · ${label}</div><h1>${htmlSafe(project.name)}</h1><div class="sub">${htmlSafe(project.client)} · ${htmlSafe(room.name)}</div></div><div class="pg">Ambiente ${ri+1}/${rooms.length}</div></div>`;
    pages.push(`<section class="page">${head('PLANTA PRINCIPAL')}<div class="visual"><img src="${plan}"/></div><div class="caption">${room.wallCount} paredes · ${(room.elements||[]).length} itens · ${(room.photos||[]).length} fotos</div></section>`);
    pages.push(`<section class="page">${head(`VISTA FRONTAL · PAREDE ${String.fromCharCode(65+chosenWall)}`)}<div class="visual front"><img src="${front}"/></div><div class="caption">Parede ${String.fromCharCode(65+chosenWall)} · ${htmlSafe(mFmt((room.lengths||[])[chosenWall]||0))} × ${htmlSafe(mFmt(room.height||2.65))}</div></section>`);
    const wallRows=(room.lengths||[]).map((v,i)=>`<div class="row"><b>Parede ${String.fromCharCode(65+i)}</b><span>${htmlSafe(mFmt(v))} × ${htmlSafe(mFmt(room.height||2.65))}</span></div>`).join('');
    const itemRows=(room.elements||[]).map(e=>{const free=isFreePlanType(e.type)||e.free===true,local=free?'Livre no ambiente':`Parede ${String.fromCharCode(65+elementWallIndex(e,room))}`;return `<div class="item"><div><b>${htmlSafe(e.type)}</b><small>${htmlSafe(local)}</small></div><span>${htmlSafe(dimensionText(e))}</span></div>`}).join('');
    pages.push(`<section class="page">${head('FICHA TÉCNICA')}<h2>Paredes</h2><div class="rows">${wallRows}</div><h2>Itens e interferências</h2><div class="items">${itemRows||'<div class="empty">Nenhum item registrado.</div>'}</div></section>`);
    const photos=(room.photos||[]).map((photo,i)=>{const src=photo?.dataUri||photo?.uri||'';return src?`<figure><img src="${htmlSafe(src)}"/><figcaption>Foto ${i+1}</figcaption></figure>`:''}).filter(Boolean).join('');
    pages.push(`<section class="page">${head('FOTOS E NOTAS')}<h2>Fotos do ambiente</h2>${photos?`<div class="photos">${photos}</div>`:'<div class="empty photoempty">Nenhuma foto adicionada ao ambiente.</div>'}<h2>Notas</h2><div class="notes">${room.notes?.trim()?htmlSafe(room.notes).replace(/\n/g,'<br>'):'Nenhuma nota registrada para este ambiente.'}</div></section>`);
  }
  return `<!doctype html><html><head><meta charset="utf-8"><style>@page{size:A4;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0;font-family:Arial,Helvetica,sans-serif;color:#101820;background:#fff}.page{width:190mm;min-height:277mm;margin:0 auto;padding:10mm 9mm;page-break-after:always;overflow:hidden}.page:last-child{page-break-after:auto}.top{display:flex;justify-content:space-between;gap:8mm;border-bottom:1.2mm solid #101820;padding-bottom:4mm;margin-bottom:6mm}.brand{font-size:9pt;color:#1677f2;font-weight:800;letter-spacing:.8pt}.top h1{font-size:20pt;margin:2mm 0 1mm;line-height:1.08}.sub,.pg,.caption{font-size:10pt;color:#64748b}.pg{white-space:nowrap}.visual{height:205mm;border:1px solid #d8e0e9;border-radius:4mm;padding:5mm;display:flex;align-items:center;justify-content:center;overflow:hidden}.visual.front{height:185mm}.visual img{display:block;max-width:100%;max-height:100%;width:auto;height:auto;object-fit:contain}.caption{margin-top:4mm;text-align:center}h2{font-size:12pt;color:#1677f2;text-transform:uppercase;letter-spacing:.6pt;margin:5mm 0 2.5mm}.rows,.items,.notes,.empty{border:1px solid #d8e0e9;border-radius:3mm;overflow:hidden}.row,.item{display:flex;justify-content:space-between;align-items:flex-start;gap:6mm;padding:3mm 4mm;border-bottom:1px solid #e8edf2;font-size:10pt}.row:last-child,.item:last-child{border-bottom:0}.item>div{min-width:45mm}.item small{display:block;color:#64748b;margin-top:1mm}.item span{max-width:100mm;text-align:right;color:#536273}.notes,.empty{padding:5mm;font-size:10pt;line-height:1.5;background:#fafbfd}.photos{display:grid;grid-template-columns:1fr 1fr;gap:4mm}.photos figure{margin:0;border:1px solid #d8e0e9;border-radius:3mm;padding:2mm}.photos img{width:100%;height:78mm;object-fit:contain;background:#f4f6f8}.photos figcaption{font-size:9pt;color:#64748b;padding-top:1mm}.photoempty{height:70mm;display:flex;align-items:center;justify-content:center;color:#64748b}</style></head><body>${pages.join('')}</body></html>`;
};
...
function MoreScreen({onBack,onNew,onIntegration}){
  return <View style={styles.screen}><Header title="Mais" subtitle="GW Medidas" onBack={onBack}/><ScrollView contentContainerStyle={styles.simplePage}>
    ...
    <View style={styles.infoCard}><View style={{flex:1}}><Text style={styles.infoTitle}>GW Medidas</Text><Text style={styles.infoText}>Medição técnica para marcenaria · versão 6.6.19</Text></View></View>
...
function GwIntegrationScreen({projects,onBack,onSynced}){
  ...
  const syncNow=async()=>{
    ...
    try{
      ...
      setLast({projects:realProjects,rooms,clients:(payload.clientes||[]).length});
      setMessage(`Sincronizado: ${realProjects} projeto(s) · ${rooms} ambiente(s).`);
    }catch(e){
...
function QuickPhotoMeasure({job,onBack,onUpdate,onSave,onFinish}){
  ...
  const [draftArea,setDraftArea]=useState(null),[pendingArea,setPendingArea]=useState(null),[areaWidth,setAreaWidth]=useState(''),[areaHeight,setAreaHeight]=useState('');
  ...
  const [photoDirty,setPhotoDirty]=useState(()=>!!activePhoto&&!activePhoto.savedAt);
  const [photoSourceOpen,setPhotoSourceOpen]=useState(false);
  const [unsavedPhotoWarningOpen,setUnsavedPhotoWarningOpen]=useState(false);
  const [selectedMarkId,setSelectedMarkId]=useState(null),[editMark,setEditMark]=useState(null),[dragHandle,setDragHandle]=useState(null);
  const zoomRef=useRef(1),offsetRef=useRef({x:0,y:0}),panStartRef=useRef({x:0,y:0}),markDragRef=useRef(null);
  ...
  const onlyNumber=v=>String(v??'').replace(/[^0-9,.]/g,'').replace(/(,|\.)(?=.*(,|\.))/g,'');
  const numberValue=v=>{const n=Number(String(v||'').replace(',','.'));return Number.isFinite(n)?n:0};
  const formatMetric=v=>{const t=String(v||'').trim().replace('.',',');return t?`${t} m`:''};
  const resetView=()=>{setZoom(1);setOffset({x:0,y:0});setTool('navigate');setAnglePoints([])};
  const dist=(a,b)=>Math.hypot(a.x-b.x,a.y-b.y);
  const markHit=(q)=>{const ms=[...(activePhoto?.marks||[])].reverse();for(const m of ms){if(m.kind==='line'){const a={x:m.x1,y:m.y1},b={x:m.x2,y:m.y2},ab=dist(a,b)||1,t=clamp(((q.x-a.x)*(b.x-a.x)+(q.y-a.y)*(b.y-a.y))/(ab*ab),0,1),d=dist(q,{x:a.x+(b.x-a.x)*t,y:a.y+(b.y-a.y)*t});if(d<.035)return m;}if(m.kind==='area'){const x1=Math.min(m.x1,m.x2),x2=Math.max(m.x1,m.x2),y1=Math.min(m.y1,m.y2),y2=Math.max(m.y1,m.y2);if(q.x>=x1-.025&&q.x<=x2+.025&&q.y>=y1-.025&&q.y<=y2+.025)return m;}if(m.kind==='angle'){if(m.points?.some(p=>dist(q,p)<.04))return m;}if(m.kind==='text'&&dist(q,{x:m.x,y:m.y})<.06)return m;}return null};
  const beginEditValue=m=>{setEditMark(m);if(m.kind==='line')setMeasureValue(m.rawValue||String(m.value||'').replace(/[^0-9,.]/g,''));else if(m.kind==='area'){setAreaWidth(m.widthValue||'');setAreaHeight(m.heightValue||'')}else if(m.kind==='text')setTextValue(m.value||'')};
  const replaceMark=nextMark=>{if(!activePhoto)return;updatePhoto({...activePhoto,marks:(activePhoto.marks||[]).map(m=>m.id===nextMark.id?nextMark:m)});setEditMark(null)};
  const updatePhoto=updated=>{const next={...job,photos:(job.photos||[]).map(p=>p.id===updated.id?updated:p),updatedAt:Date.now()};onUpdate(next);setActivePhoto(updated);setPhotoDirty(true);return next};
  const addPhoto=async(source='camera')=>{
    // Abra a câmera/galeria primeiro. No Safari/PWA, aguardar um salvamento antes de
    // abrir o seletor pode consumir o gesto do usuário e impedir a câmera de abrir.
    let r;
    try{
      if(source==='camera'){
        const perm=await ImagePicker.requestCameraPermissionsAsync();
        if(!perm.granted){Alert.alert('Câmera','Autorize o acesso à câmera para fazer a medição rápida.');return;}
        r=await ImagePicker.launchCameraAsync({mediaTypes:['images'],quality:.58,base64:true});
      }else{
        r=await ImagePicker.launchImageLibraryAsync({mediaTypes:['images'],quality:.58,base64:true});
      }
    }catch(e){
      console.warn('quick add photo',e);
      Alert.alert('Foto','Não consegui abrir a câmera. Tente novamente.');
      return;
    }
    if(r?.canceled||!r?.assets?.[0])return;
    const a=r.assets[0];
    const photo={id:uid(),uri:a.uri,dataUri:a.base64?`data:${a.mimeType||'image/jpeg'};base64,${a.base64}`:null,marks:[],createdAt:Date.now()};
    const next={...job,photos:[...(job.photos||[]),photo],updatedAt:Date.now()};
    // A nova foto entra no mesmo levantamento; a persistência ocorre já com as duas fotos.
    onUpdate(next);
    setActivePhoto(photo);
    setPhotoDirty(true);
    resetView();
    try{await onSave?.(next,true)}catch(e){console.warn('quick save after add photo',e)}
  };
  ...
  const drawPan=useMemo(()=>Gesture.Pan().minDistance(2).runOnJS(true).onBegin(e=>{if(tool==='measure'){const q=localPoint(e);setDraftLine({x1:q.x,y1:q.y,x2:q.x,y2:q.y})}else if(tool==='area'){const q=localPoint(e);setDraftArea({x1:q.x,y1:q.y,x2:q.x,y2:q.y})}else if(tool==='navigate'){const q=localPoint(e);let hit=(activePhoto?.marks||[]).find(m=>m.id===selectedMarkId);if(!hit)hit=markHit(q);let h=null;if(hit){setSelectedMarkId(hit.id);if(hit.kind==='line'){if(dist(q,{x:hit.x1,y:hit.y1})<.045)h='p1';else if(dist(q,{x:hit.x2,y:hit.y2})<.045)h='p2'}else if(hit.kind==='area'){const pts=[['p1',{x:hit.x1,y:hit.y1}],['p2',{x:hit.x2,y:hit.y1}],['p3',{x:hit.x2,y:hit.y2}],['p4',{x:hit.x1,y:hit.y2}]];h=pts.find(v=>dist(q,v[1])<.045)?.[0]||'move'}else if(hit.kind==='angle'){h=hit.points?.map((p,i)=>[String(i),p]).find(v=>dist(q,v[1])<.045)?.[0]||null}else if(hit.kind==='text')h='move';if(h==='move')markDragRef.current={start:q,mark:{...hit}}}setDragHandle(h);panStartRef.current=offsetRef.current}}).onUpdate(e=>{if(tool==='measure'){const q=localPoint(e);setDraftLine(d=>d?{...d,x2:q.x,y2:q.y}:d)}else if(tool==='area'){const q=localPoint(e);setDraftArea(d=>d?{...d,x2:q.x,y2:q.y}:d)}else if(tool==='navigate'){const hit=(activePhoto?.marks||[]).find(m=>m.id===selectedMarkId);if(hit&&dragHandle){const q=localPoint(e);let nm={...hit};if(dragHandle==='move'&&markDragRef.current){const base=markDragRef.current.mark,dx=q.x-markDragRef.current.start.x,dy=q.y-markDragRef.current.start.y;if(hit.kind==='text'){nm={...base,x:clamp(base.x+dx,0.02,0.98),y:clamp(base.y+dy,0.02,0.98)}}else if(hit.kind==='area'){const minX=Math.min(base.x1,base.x2),maxX=Math.max(base.x1,base.x2),minY=Math.min(base.y1,base.y2),maxY=Math.max(base.y1,base.y2),safeDx=clamp(dx,-minX,1-maxX),safeDy=clamp(dy,-minY,1-maxY);nm={...base,x1:base.x1+safeDx,x2:base.x2+safeDx,y1:base.y1+safeDy,y2:base.y2+safeDy}}}else if(hit.kind==='line'){if(dragHandle==='p1'){nm.x1=q.x;nm.y1=q.y}else{nm.x2=q.x;nm.y2=q.y}}else if(hit.kind==='area'){if(dragHandle==='p1'){nm.x1=q.x;nm.y1=q.y}else if(dragHandle==='p2'){nm.x2=q.x;nm.y1=q.y}else if(dragHandle==='p3'){nm.x2=q.x;nm.y2=q.y}else{nm.x1=q.x;nm.y2=q.y}}else if(hit.kind==='angle'){nm.points=hit.points.map((p,i)=>i===Number(dragHandle)?q:p);const [a,b,c]=nm.points,v1={x:a.x-b.x,y:a.y-b.y},v2={x:c.x-b.x,y:c.y-b.y},den=Math.hypot(v1.x,v1.y)*Math.hypot(v2.x,v2.y),deg=den?Math.acos(clamp((v1.x*v2.x+v1.y*v2.y)/den,-1,1))*180/Math.PI:0;nm.value=`${deg.toFixed(1).replace('.',',')}°`}updatePhoto({...activePhoto,marks:(activePhoto.marks||[]).map(m=>m.id===nm.id?nm:m)})}else setOffset({x:panStartRef.current.x+e.translationX,y:panStartRef.current.y+e.translationY})}}).onEnd(e=>{if(tool==='measure'){setDraftLine(d=>{if(!d)return null;const q=localPoint(e),line={...d,x2:q.x,y2:q.y};if(Math.hypot(line.x2-line.x1,line.y2-line.y1)>.02){setPendingLine(line);setMeasureValue('')}return null})}else if(tool==='area'){setDraftArea(d=>{if(!d)return null;const q=localPoint(e),a={...d,x2:q.x,y2:q.y};if(Math.abs(a.x2-a.x1)>.02&&Math.abs(a.y2-a.y1)>.02){setPendingArea(a);setAreaWidth('');setAreaHeight('')}return null})}setDragHandle(null);markDragRef.current=null}),[tool,boxW,boxH,activePhoto,selectedMarkId,dragHandle]);
  const toolTap=useMemo(()=>Gesture.Tap().maxDistance(8).runOnJS(true).onEnd((e,ok)=>{if(!ok)return;const q=localPoint(e);if(tool==='angle'){setAnglePoints(prev=>{const pts=[...prev,q];if(pts.length===3){const [a,b,c]=pts,v1={x:a.x-b.x,y:a.y-b.y},v2={x:c.x-b.x,y:c.y-b.y},dot=v1.x*v2.x+v1.y*v2.y,den=Math.hypot(v1.x,v1.y)*Math.hypot(v2.x,v2.y),deg=den?Math.acos(clamp(dot/den,-1,1))*180/Math.PI:0;const mark={id:uid(),kind:'angle',points:pts,value:`${deg.toFixed(1).replace('.',',')}°`};const updated={...activePhoto,marks:[...(activePhoto.marks||[]),mark]};updatePhoto(updated);setTool('navigate');return []}return pts})}else if(tool==='text'){setTextPoint(q);setTextValue('');setTextOpen(true)}else if(tool==='navigate'){const hit=markHit(q);setSelectedMarkId(hit?.id||null);if(hit)beginEditValue(hit)}}),[tool,activePhoto,job]);
  const pinch=useMemo(()=>{let base=1;return Gesture.Pinch().runOnJS(true).onBegin(()=>{base=zoomRef.current}).onUpdate(e=>{const z=clamp(base*e.scale,1,5);setZoom(z);if(z<=1.02)setOffset({x:0,y:0})}).onEnd(()=>{const z=zoomRef.current;if(z<=1.14){setZoom(1);setOffset({x:0,y:0})}})},[]);
  const photoGesture=useMemo(()=>Gesture.Simultaneous(drawPan,pinch,toolTap),[drawPan,pinch,toolTap]);
  const confirmMeasure=()=>{if(!activePhoto||(!pendingLine&&editMark?.kind!=='line'))return;const raw=onlyNumber(measureValue);if(!numberValue(raw))return;const value=formatMetric(raw);if(editMark?.kind==='line')replaceMark({...editMark,value,rawValue:raw,unit:'m'});else updatePhoto({...activePhoto,marks:[...(activePhoto.marks||[]),{id:uid(),kind:'line',...pendingLine,value,rawValue:raw,unit:'m'}]});setPendingLine(null);setMeasureValue('');setTool('navigate')};
  const confirmArea=()=>{if(!activePhoto||(!pendingArea&&editMark?.kind!=='area'))return;const w=numberValue(areaWidth),h=numberValue(areaHeight);if(!w||!h)return;const sqm=w*h,value=`${sqm.toFixed(2).replace('.',',')} m²`;if(editMark?.kind==='area')replaceMark({...editMark,value,widthValue:areaWidth.replace('.',','),heightValue:areaHeight.replace('.',','),sqm,unit:'m'});else updatePhoto({...activePhoto,marks:[...(activePhoto.marks||[]),{id:uid(),kind:'area',...pendingArea,value,widthValue:areaWidth.replace('.',','),heightValue:areaHeight.replace('.',','),sqm,unit:'m'}]});setPendingArea(null);setAreaWidth('');setAreaHeight('');setTool('navigate')};
  const confirmText=()=>{if(!activePhoto||(!textPoint&&editMark?.kind!=='text')||!textValue.trim())return;if(editMark?.kind==='text'){replaceMark({...editMark,value:textValue.trim()});setTextOpen(false);setTextValue('');return}updatePhoto({...activePhoto,marks:[...(activePhoto.marks||[]),{id:uid(),kind:'text',x:textPoint.x,y:textPoint.y,value:textValue.trim()}]});setTextOpen(false);setTextPoint(null);setTextValue('');setTool('navigate')};
  ...
  const latestJob=(status='in_progress')=>({...job,photos:(job.photos||[]).map(p=>p.id===activePhoto?.id?activePhoto:p),status,...(status==='completed'?{completedAt:Date.now()}:{}),updatedAt:Date.now()});
  const handleSavePhoto=async()=>{if(saveState!=='idle'||!activePhoto)return;setSaveState('saving');try{const savedPhoto={...activePhoto,savedAt:Date.now()};const savedJob={...latestJob('in_progress'),photos:(job.photos||[]).map(p=>p.id===savedPhoto.id?savedPhoto:p),updatedAt:Date.now()};await onSave?.(savedJob);setActivePhoto(savedPhoto);onUpdate(savedJob);setPhotoDirty(false);setSaveState('saved');setTimeout(()=>setSaveState('idle'),1400)}catch(e){console.warn('quick save',e);setSaveState('idle');Alert.alert('Não foi possível salvar','A foto ficou mantida nesta tela. Tente novamente.')}};
  const requestMorePhoto=()=>{
    // Proteção explícita: qualquer alteração na foto atual precisa ser salva antes
    // de permitir câmera/biblioteca. Usamos modal próprio para funcionar igual
    // no iPhone/Safari/PWA, sem depender do Alert nativo do navegador.
    if(photoDirty){setTool('navigate');setUnsavedPhotoWarningOpen(true);return;}
    setTool('navigate');setPhotoSourceOpen(true);
  };
  const handleFinish=async()=>{if(saveState!=='idle')return;setSaveState('finishing');try{await onFinish?.(latestJob('completed'))}catch(e){console.warn('quick finish',e);setSaveState('idle');Alert.alert('Não foi possível finalizar','O levantamento não foi fechado para evitar perda de dados.')}};
  ...
  <Modal visible={textOpen||editMark?.kind==='text'} transparent animationType="fade" onRequestClose={()=>{setTextOpen(false);setTextPoint(null);setEditMark(null);setTool('navigate')}}><View style={styles.modalShade}><View style={styles.modalCard}><Text style={styles.modalTitle}>Adicionar texto</Text><Text style={styles.modalText}>Escreva uma identificação curta para este ponto da foto.</Text><TextInput autoFocus value={textValue} onChangeText={setTextValue} placeholder="Ex.: tomada 20 A" placeholderTextColor="#9AA6B5" style={[styles.input,{marginTop:14,fontSize:18,fontWeight:'700'}]} onSubmitEditing={confirmText}/><View style={{flexDirection:'row',gap:8,marginTop:14}}><Button secondary title="Cancelar" onPress={()=>{setTextOpen(false);setTextPoint(null);setEditMark(null);setTool('navigate')}}/><Button disabled={!textValue.trim()} title="Adicionar" onPress={confirmText}/></View></View></View></Modal>
  <Modal visible={!!pendingLine||editMark?.kind==='line'} transparent animationType="fade" onRequestClose={()=>{setPendingLine(null);setEditMark(null);setTool('navigate')}}><View style={styles.modalShade}><View style={styles.modalCard}><Text style={styles.modalTitle}>Qual é a medida?</Text><Text style={styles.modalText}>Digite o valor medido. Ex.: 2,47 m</Text><TextInput autoFocus value={measureValue} onChangeText={v=>setMeasureValue(onlyNumber(v))} keyboardType="decimal-pad" inputMode="decimal" placeholder="2,47" placeholderTextColor="#9AA6B5" style={[styles.input,{marginTop:14,fontSize:24,fontWeight:'800'}]} onSubmitEditing={confirmMeasure}/><Text style={{fontSize:11,color:MUTED,marginTop:6}}>Unidade: metros (m)</Text><View style={{flexDirection:'row',gap:8,marginTop:14}}><Button secondary title="Cancelar" onPress={()=>{setPendingLine(null);setEditMark(null);setTool('navigate')}}/><Button disabled={!measureValue.trim()} title="Salvar medida" onPress={confirmMeasure}/></View></View></View></Modal>
  <Modal visible={!!pendingArea||editMark?.kind==='area'} transparent animationType="fade" onRequestClose={()=>{setPendingArea(null);setEditMark(null);setTool('navigate')}}><View style={styles.modalShade}><View style={styles.modalCard}><Text style={styles.modalTitle}>Medidas da área</Text><Text style={styles.modalText}>Informe os dois lados. O GW calcula a área automaticamente.</Text><View style={{flexDirection:'row',gap:10,marginTop:14}}><View style={{flex:1}}><Text style={{fontSize:10,fontWeight:'900',color:MUTED,marginBottom:5}}>LARGURA (m)</Text><TextInput autoFocus value={areaWidth} onChangeText={v=>setAreaWidth(onlyNumber(v))} keyboardType="decimal-pad" inputMode="decimal" placeholder="2,40" placeholderTextColor="#9AA6B5" style={[styles.input,{fontSize:20,fontWeight:'800'}]}/></View><View style={{flex:1}}><Text style={{fontSize:10,fontWeight:'900',color:MUTED,marginBottom:5}}>ALTURA (m)</Text><TextInput value={areaHeight} onChangeText={v=>setAreaHeight(onlyNumber(v))} keyboardType="decimal-pad" inputMode="decimal" placeholder="1,80" placeholderTextColor="#9AA6B5" style={[styles.input,{fontSize:20,fontWeight:'800'}]}/></View></View><View style={{marginTop:12,padding:12,borderRadius:12,backgroundColor:BLUE_SOFT,alignItems:'center'}}><Text style={{fontSize:10,fontWeight:'900',color:BLUE}}>ÁREA CALCULADA</Text><Text style={{fontSize:25,fontWeight:'900',color:INK,marginTop:2}}>{numberValue(areaWidth)&&numberValue(areaHeight)?`${(numberValue(areaWidth)*numberValue(areaHeight)).toFixed(2).replace('.',',')} m²`:'— m²'}</Text></View><View style={{flexDirection:'row',gap:8,marginTop:14}}><Button secondary title="Cancelar" onPress={()=>{setPendingArea(null);setEditMark(null);setTool('navigate')}}/><Button disabled={!numberValue(areaWidth)||!numberValue(areaHeight)} title="Salvar área" onPress={confirmArea}/></View></View></View></Modal></View>
}
...
export default function App(){
  ...
  const deleteQuickJob=id=>confirmDelete('Excluir medição rápida','Excluir esta medição rápida e todas as fotos e marcações dela?',async()=>{const next=quickJobs.filter(q=>q.id!==id);const cleanNext=await dehydrateQuickJobs(next);await AsyncStorage.setItem(QUICK_STORE,JSON.stringify(cleanNext));setQuickJobs(next);if(quickJob?.id===id)setQuickJob(null)});
  ...
  const openLinkCurrentToGw=async()=>{
    ...
    try{
      ...
      const all=[...projectsGw].sort((a,b)=>Number(norm(b.client)===norm(project.client))-Number(norm(a.client)===norm(project.client)));
  ...
  const linkCurrentToGw=async target=>{
    ...
    const next=projects.map(p=>p.id!==project.id?p:{...p,client:target.client||p.client,name:target.name||p.name,gwImported:true,gwSourceType:target.type,gwBudgetId:null,gwProjectId:target.id,gwCompanyId:target.companyId,gwLinkedFromLocal:true,gwSyncAt:Date.now(),updatedAt:Date.now()});
    await persist(next);
    setGwLinkModal({visible:false,targets:[],error:''});
    Alert.alert('Vínculo criado','Esta medição agora está vinculada a um projeto do GW Assistente.');
  };

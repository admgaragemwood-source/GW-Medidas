...
function QuickPhotoMeasure({job,onBack,onUpdate,onSave,onFinish}){
  ...
  const handleSavePhoto=async()=>{if(saveState!=='idle')return;setSaveState('saving');try{await onSave?.(latestJob('in_progress'));setSaveState('saved');setTimeout(()=>setSaveState('idle'),1400)}catch(e){console.warn('quick save',e);setSaveState('idle');Alert.alert('Não foi possível salvar','A foto ficou mantida nesta tela. Tente novamente.')}};

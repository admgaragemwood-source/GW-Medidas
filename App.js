...
// GW Medidas 6.6.7 — refinamento técnico da medição rápida
...
function QuickPhotoMeasure({job,onBack,onUpdate,onSave,onFinish}){
  ...
  const confirmMeasure=()=>{if(!activePhoto||!pendingLine)return;const raw=onlyNumber(measureValue);if(!numberValue(raw))return;const value=formatMetric(raw);updatePhoto({...activePhoto,marks:[...(activePhoto.marks||[]),{id:uid(),kind:'line',...pendingLine,value,rawValue:raw,unit:'m'}]});setPendingLine(null);setMeasureValue('');setTool('navigate')};
  const confirmArea=()=>{if(!activePhoto||!pendingArea)return;const w=numberValue(areaWidth),h=numberValue(areaHeight);if(!w||!h)return;const sqm=w*h,value=`${sqm.toFixed(2).replace('.',',')} m²`;updatePhoto({...activePhoto,marks:[...(activePhoto.marks||[]),{id:uid(),kind:'area',...pendingArea,value,widthValue:areaWidth.replace('.',','),heightValue:areaHeight.replace('.',','),sqm,unit:'m'}]});setPendingArea(null);setAreaWidth('');setAreaHeight('');setTool('navigate')};
...
export default function App(){
  ...
  const persistQuick=async(job)=>{const compactJob={...job,photos:(job.photos||[]).map(p=>{const persistentUri=p.dataUri||p.uri;const {dataUri,...rest}=p;return {...rest,uri:persistentUri}})};const next=[compactJob,...quickJobs.filter(q=>q.id!==job.id)];setQuickJobs(next);setQuickJob(compactJob);try{await AsyncStorage.setItem('gw-medidas-quick-v1',JSON.stringify(next))}catch(e){console.warn('persistQuick storage',e);const lightNext=next.map(q=>({...q,photos:(q.photos||[]).map(p=>({...p,uri:typeof p.uri==='string'&&p.uri.startsWith('data:')?null:p.uri}))}));await AsyncStorage.setItem('gw-medidas-quick-v1',JSON.stringify(lightNext));}return compactJob};
  ...
  else if(screen==='quickPhoto')content=<QuickPhotoMeasure job={quickJob} onBack={()=>setScreen('home')} onUpdate={setQuickJob} onSave={async (j,silent=false)=>{await persistQuick(j);if(!silent)Alert.alert('Salvo','Foto salva. Você pode continuar medindo ou adicionar outra foto.')}} onFinish={async j=>{await persistQuick(j);setQuickJob(null);setScreen('projects')}}/>;

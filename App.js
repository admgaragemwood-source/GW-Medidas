...
function Home({ projects, quickJobs=[], onQuick, onNew, onOpen, onOpenQuick, onDelete, onClients, onHelp, onMore }) {
  ...
  return <View style={styles.screen}><View style={styles.projectsHeader}><Pressable onPress={()=>setMenuOpen(true)} hitSlop={10} style={styles.projectsMenuBtn}><Text style={styles.projectsMenu}>☰</Text></Pressable><Text style={styles.projectsHeaderTitle}>Minhas medições</Text><Pressable onPress={onNew} style={styles.projectsPlus}><Text style={styles.projectsPlusText}>＋</Text></Pressable></View>
    <View style={styles.homeModeWrap}><Text style={styles.homeModeTitle}>Como você quer medir?</Text><View style={styles.homeModeRow}><Pressable onPress={onQuick} style={[styles.homeModeCard,styles.homeModeQuick]}><Text style={styles.homeModeIcon}>⚡</Text><Text style={styles.homeModeCardTitle}>Medição rápida</Text><Text style={styles.homeModeCardText}>Foto + cotas</Text></Pressable><Pressable onPress={onNew} style={styles.homeModeCard}><Text style={styles.homeModeIcon}>⌗</Text><Text style={styles.homeModeCardTitle}>Medição completa</Text><Text style={styles.homeModeCardText}>Levantamento técnico</Text></Pressable></View></View>
    <View style={styles.searchBox}><Text style={styles.searchIcon}>⌕</Text><TextInput value={query} onChangeText={setQuery} placeholder="Buscar medições..." placeholderTextColor="#8B94A2" style={styles.searchInput}/></View>
    ...
    <ScrollView style={{flex:1}} contentContainerStyle={styles.homeList} showsVerticalScrollIndicator={false}>{quickJobs.map(q=><Pressable key={q.id} onPress={()=>onOpenQuick?.(q.id)} style={styles.quickSavedCard}><View style={styles.quickSavedThumb}>{q.photos?.[0]?.uri?<Image source={{uri:q.photos[0].uri}} style={StyleSheet.absoluteFillObject} resizeMode="cover"/>:<Text style={styles.quickSavedIcon}>📷</Text>}</View><View style={{flex:1}}><Text style={styles.projectTitleSmall}>{q.project}</Text><Text style={styles.projectMetaSmall}>{q.client} · {q.photos?.length||0} foto{(q.photos?.length||0)!==1?'s':''}</Text><View style={styles.quickSavedPill}><Text style={styles.quickSavedPillText}>Medição rápida</Text></View></View><Text style={styles.chevSmall}>›</Text></Pressable>)}{visible.length?visible.map((p,i)=><View key={p.id} style={styles.projectCardMock}><Pressable onPress={()=>onOpen(p.id)} style={styles.projectOpenArea}><View style={styles.projectThumb}><View style={styles.thumbWall}/><View style={styles.thumbCabinet}/></View><View style={{flex:1}}><Text style={styles.projectTitleSmall}>{p.name}</Text><Text style={styles.projectMetaSmall}>{p.client} · {p.rooms?.length||0} ambiente{(p.rooms?.length||0)!==1?'s':''}</Text><View style={[styles.statusPill,p.gwSourceType==='orcamento'&&styles.statusPillBudget,p.gwSourceType==='projeto'&&styles.statusPillProject]}><Text style={[styles.statusText,p.gwSourceType==='orcamento'&&styles.statusTextBudget,p.gwSourceType==='projeto'&&styles.statusTextProject]}>{p.gwSourceType==='orcamento'?`Orçamento · ${p.gwBudgetSnapshot?.status||'Rascunho'}`:p.gwSourceType==='projeto'?'Projeto aprovado':'Medição local'}</Text></View></View><Text style={styles.chevSmall}>›</Text></Pressable><Pressable hitSlop={8} onPress={()=>onDelete(p.id)} style={styles.deleteIconBtn}><Text style={styles.deleteIconText}>×</Text></Pressable></View>):<View style={styles.emptyHome}><Text style={styles.emptyHomeIcon}>⌗</Text><Text style={styles.emptyHomeTitle}>Nenhum projeto ainda</Text><Text style={styles.emptyHomeText}>Toque no + para criar sua primeira medição.</Text></View>}</ScrollView>
...
function MoreScreen({onBack,onNew,onIntegration}){
  return <View style={styles.screen}><Header title="Mais" subtitle="GW Medidas" onBack={onBack}/><ScrollView contentContainerStyle={styles.simplePage}>
    ...
    <View style={styles.infoCard}><View style={{flex:1}}><Text style={styles.infoTitle}>GW Medidas</Text><Text style={styles.infoText}>Medição técnica para marcenaria · versão 6.5.4</Text></View></View>
...
function QuickPhotoMeasure({job,onBack,onUpdate,onSave,onFinish}){
  ...
  const saveJob=next=>{onUpdate(next);setActivePhoto(next.photos?.[next.photos.length-1]||activePhoto)};
  const addPhoto=async(source='camera')=>{let r;if(source==='camera'){const perm=await ImagePicker.requestCameraPermissionsAsync();if(!perm.granted){Alert.alert('Câmera','Autorize o acesso à câmera para fazer a medição rápida.');return;}r=await ImagePicker.launchCameraAsync({mediaTypes:['images'],quality:.82,base64:true});}else r=await ImagePicker.launchImageLibraryAsync({mediaTypes:['images'],quality:.82,base64:true});if(r?.canceled)return;const a=r.assets[0],photo={id:uid(),uri:a.uri,dataUri:a.base64?`data:${a.mimeType||'image/jpeg'};base64,${a.base64}`:null,marks:[],createdAt:Date.now()};const next={...job,photos:[...(job.photos||[]),photo],updatedAt:Date.now()};onUpdate(next);setActivePhoto(photo)};
  const pan=useMemo(()=>Gesture.Pan().minDistance(4).runOnJS(true).onBegin(e=>setDraftLine({x1:clamp(e.x/boxW,0,1),y1:clamp(e.y/boxH,0,1),x2:clamp(e.x/boxW,0,1),y2:clamp(e.y/boxH,0,1)})).onUpdate(e=>setDraftLine(d=>d?{...d,x2:clamp(e.x/boxW,0,1),y2:clamp(e.y/boxH,0,1)}:d)).onEnd(e=>{setDraftLine(d=>{if(!d)return null;const line={...d,x2:clamp(e.x/boxW,0,1),y2:clamp(e.y/boxH,0,1)};if(Math.hypot(line.x2-line.x1,line.y2-line.y1)>.04){setPendingLine(line);setMeasureValue('');}return null;});}),[boxW,boxH]);
  const confirmMeasure=()=>{if(!activePhoto||!pendingLine)return;const value=measureValue.trim();if(!value)return;const mark={id:uid(),...pendingLine,value};const photos=(job.photos||[]).map(p=>p.id===activePhoto.id?{...p,marks:[...(p.marks||[]),mark]}:p);const updated={...activePhoto,marks:[...(activePhoto.marks||[]),mark]};onUpdate({...job,photos,updatedAt:Date.now()});setActivePhoto(updated);setPendingLine(null);setMeasureValue('')};
  const deleteMark=id=>{if(!activePhoto)return;const updated={...activePhoto,marks:(activePhoto.marks||[]).filter(m=>m.id!==id)};onUpdate({...job,photos:(job.photos||[]).map(p=>p.id===updated.id?updated:p),updatedAt:Date.now()});setActivePhoto(updated)};
  if(!activePhoto)return <View style={styles.screen}><Header title="Medição rápida" subtitle={`${job.client} · ${job.project}`} onBack={onBack}/><View style={styles.quickCameraEmpty}><Text style={styles.quickCameraIcon}>📷</Text><Text style={styles.quickCameraTitle}>Fotografe a parede</Text><Text style={styles.quickCameraText}>Depois, arraste o dedo sobre a foto para criar cada linha de medida.</Text><Button title="Tirar foto" onPress={()=>addPhoto('camera')}/><Button secondary title="Escolher da galeria" onPress={()=>addPhoto('gallery')}/></View></View>;
  const allLines=[...(activePhoto.marks||[]),...(draftLine?[{...draftLine,id:'draft',value:''}]:[])];
  return <View style={styles.screen}><Header title="Medição rápida" subtitle={`${job.client} · ${job.project}`} onBack={onBack}/><ScrollView contentContainerStyle={{paddingBottom:110}}><View style={styles.quickToolbar}><View style={{flex:1}}><Text style={styles.quickToolbarText}>＋ Adicionar medida</Text><Text style={styles.quickToolbarSub}>Arraste de um ponto ao outro sobre a foto.</Text></View><Pressable onPress={()=>addPhoto('camera')} style={styles.quickNewPhoto}><Text style={styles.quickNewPhotoText}>＋ Foto</Text></Pressable></View>
    <View style={[styles.quickPhotoStage,{width:boxW,height:boxH}]}><Image source={{uri:activePhoto.uri}} style={StyleSheet.absoluteFillObject} resizeMode="contain"/><GestureDetector gesture={pan}><View style={StyleSheet.absoluteFillObject}><Svg width={boxW} height={boxH}>{allLines.map(m=>{const x1=m.x1*boxW,y1=m.y1*boxH,x2=m.x2*boxW,y2=m.y2*boxH,mx=(x1+x2)/2,my=(y1+y2)/2;return <React.Fragment key={m.id}><Line x1={x1} y1={y1} x2={x2} y2={y2} stroke={m.id==='draft'?BLUE:'#1677F2'} strokeWidth="3"/><Circle cx={x1} cy={y1} r="5" fill={WHITE} stroke={BLUE} strokeWidth="3"/><Circle cx={x2} cy={y2} r="5" fill={WHITE} stroke={BLUE} strokeWidth="3"/>{m.value?<><Rect x={mx-36} y={my-13} width="72" height="26" rx="7" fill={WHITE} stroke={BLUE}/><SvgText x={mx} y={my+5} fontSize="12" fontWeight="800" textAnchor="middle" fill={INK}>{m.value}</SvgText></>:null}</React.Fragment>})}</Svg></View></GestureDetector></View>
    <View style={styles.quickMarksList}>{(activePhoto.marks||[]).map((m,i)=><View key={m.id} style={styles.quickMarkRow}><Text style={styles.quickMarkIndex}>{i+1}</Text><Text style={styles.quickMarkValue}>{m.value}</Text><Pressable onPress={()=>deleteMark(m.id)}><Text style={styles.quickMarkDelete}>Excluir</Text></Pressable></View>)}</View>
    {(job.photos||[]).length>1?<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.quickThumbs}>{job.photos.map((p,i)=><Pressable key={p.id} onPress={()=>setActivePhoto(p)} style={[styles.quickThumbWrap,p.id===activePhoto.id&&styles.quickThumbOn]}><Image source={{uri:p.uri}} style={styles.quickThumb}/><Text style={styles.quickThumbText}>Foto {i+1}</Text></Pressable>)}</ScrollView>:null}
  </ScrollView><View style={styles.quickBottomActions}><Pressable onPress={()=>addPhoto('camera')} style={styles.quickBottomSecondary}><Text style={styles.quickBottomSecondaryText}>＋ Outra foto</Text></Pressable><Pressable onPress={()=>onSave?.(job)} style={styles.quickBottomSave}><Text style={styles.quickBottomSaveText}>Salvar</Text></Pressable><Pressable onPress={()=>onFinish?.(job)} style={styles.quickBottomFinish}><Text style={styles.quickBottomFinishText}>Salvar e sair</Text></Pressable></View><Modal visible={!!pendingLine} transparent animationType="fade" onRequestClose={()=>setPendingLine(null)}><View style={styles.modalShade}><View style={styles.modalCard}><Text style={styles.modalTitle}>Qual é a medida?</Text><Text style={styles.modalText}>Digite o valor medido. Ex.: 2,47 m</Text><TextInput autoFocus value={measureValue} onChangeText={setMeasureValue} placeholder="2,47 m" placeholderTextColor="#9AA6B5" style={[styles.input,{marginTop:14,fontSize:24,fontWeight:'800'}]} onSubmitEditing={confirmMeasure}/><View style={{flexDirection:'row',gap:8,marginTop:14}}><Button secondary title="Cancelar" onPress={()=>setPendingLine(null)}/><Button disabled={!measureValue.trim()} title="Salvar medida" onPress={confirmMeasure}/></View></View></View></Modal></View>
}
...
export default function App(){
  ...
  else if(screen==='quickPhoto')content=<QuickPhotoMeasure job={quickJob} onBack={()=>setScreen('home')} onUpdate={setQuickJob} onSave={async j=>{await persistQuick(j);Alert.alert('Salvo','Medição rápida salva. Você pode continuar medindo.')}} onFinish={async j=>{await persistQuick(j);setScreen('home')}}/>;
  else if(screen==='home')content=<Home projects={projects} quickJobs={quickJobs} onQuick={()=>{setQuickJob(null);setScreen('quickForm')}} onOpenQuick={id=>{const q=quickJobs.find(x=>x.id===id);if(q){setQuickJob(q);setScreen('quickPhoto')}}} onNew={()=>setScreen('new')} onOpen={id=>{setActiveProjectId(id);setScreen('project')}} onDelete={deleteProject} onClients={()=>setScreen('clients')} onHelp={()=>setScreen('help')} onMore={()=>setScreen('more')}/>;
  ...
  else if(screen==='project')content=<ProjectScreen project={project} onBack={()=>setScreen('home')} onOpenRoom={id=>{setActiveRoomId(id);setScreen('measure')}} onNewRoom={()=>newRoomForProject(project)} onDeleteRoom={deleteRoom} onDeleteProject={()=>deleteProject(project.id)} onExportProject={()=>setScreen('projectExport')} onSendToGw={sendCurrentToGw} onLinkToGw={openLinkCurrentToGw} gwSending={gwSending} gwLinking={gwLinking}/>;
